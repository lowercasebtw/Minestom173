...
public class MapGenCaves extends MapGenBase {
    ...
    protected void generate(final WorldContext worldContext, int i, int j, double d0, double d1, double d2) {
        this.generate(worldContext, i, j, d0, d1, d2, 1.0F + this.random.nextFloat() * 6.0F, 0.0F, 0.0F, -1, -1, 0.5D);
    }

    protected void generate(final WorldContext worldContext, int i, int j, double d0, double d1, double d2, float f, float f1, float f2, int k, int l, double d3) {
        double d4 = i * 16 + 8;
        double d5 = j * 16 + 8;
        float f3 = 0.0F;
        float f4 = 0.0F;
        ...
        for (boolean flag1 = random.nextInt(6) == 0; k < l; ++k) {
            ...
            if (!flag && k == j1 && f > 1.0F) {
                this.generate(worldContext, i, j, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 - 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                this.generate(worldContext, i, j, d0, d1, d2, random.nextFloat() * 0.5F + 0.5F, f1 + 1.5707964F, f2 / 3.0F, k, l, 1.0D);
                return;
            }
                    ...
                    for (j3 = k1; !flag2 && j3 < l1; ++j3) {
                        for (int k3 = k2; !flag2 && k3 < l2; ++k3) {
                            for (int l3 = j2 + 1; !flag2 && l3 >= i2 - 1; --l3) {
                                i3 = (j3 * 16 + k3) * 128 + l3;
                                if (l3 < 128) {
                                    if (worldContext.getBlock(i3).compare(Block.WATER)) {
                                        flag2 = true;
                                    }
                                    ...
                                    for (int j4 = j2 - 1; j4 >= i2; --j4) {
                                        ...
                                        if (d14 > -0.7D && d12 * d12 + d14 * d14 + d13 * d13 < 1.0D) {
                                            ...
                                            if (b0 == Block.STONE || b0 == Block.DIRT || b0 == Block.GRASS_BLOCK) {
                                                if (j4 < 10) {
                                                    worldContext.setBlock(i4, Block.LAVA);
                                                } else {
                                                    worldContext.setBlock(i4, Block.AIR);
                                                    if (flag3 && worldContext.getBlock(i4 - 1) == Block.DIRT) {
                                                        worldContext.setBlock(i4 - 1, Block.GRASS_BLOCK);
                                                    }
                                                }
                                            }
                                        }
    ...
    protected void generate(final WorldContext worldContext, final int minHeight, final int maxHeight, final int i, final int j, final int k, final int l) {
        ...
        for (int j1 = 0; j1 < i1; ++j1) {
            ...
            if (this.random.nextInt(4) == 0) {
                this.generate(worldContext, k, l, d0, d1, d2);
            ...
            for (int l1 = 0; l1 < k1; ++l1) {
                ...
                this.generate(worldContext, k, l, d0, d1, d2, f2, f, f1, 0, 0, 1.0D);
            }
        }
    }
}
