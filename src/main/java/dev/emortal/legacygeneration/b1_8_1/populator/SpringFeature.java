...
public class SpringFeature implements WorldGenerator {
    ...
    public boolean populate(Level level, Random random, int i, int i2, int i3) {
        if (level.getTile(i, i2 + 1, i3) != Tile.STONE.id) {
            return false;
        } else if (level.getTile(i, i2 - 1, i3) != Tile.STONE.id) {
            return false;
        } else if (level.getTile(i, i2, i3) != 0 && level.getTile(i, i2, i3) != Tile.STONE.id) {
            return false;
        } else {
            int var6 = 0;
            if (level.getTile(i - 1, i2, i3) == Tile.STONE.id) {
                var6++;
            }
            ...
            if (level.getTile(i, i2, i3 - 1) == Tile.STONE.id) {
                var6++;
            }
